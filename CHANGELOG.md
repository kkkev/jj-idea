...
- The bookmarks panel's "@" row and "Unbookmarked heads" rows no longer truncate a long bookmark name at the same fixed 30-character cap as the toolbar widget — they now show the full name, and in every case the trailing "+n" is never dropped by truncation. ([#107](https://github.com/kkkev/jj-idea/issues/107))
...
- The log table no longer shows a blank, unlabeled header row above the columns. Columns can still be resized by dragging their border and reordered by dragging horizontally, directly in the rows (matching Git's own log view) — including the previously-unresizable last (Date) column. Also fixed Date coming out slightly too narrow under "Fit columns to window width". ([#78](https://github.com/kkkev/jj-idea/issues/78))
...
- The conflicted-file editor banner now reflects jj's own incremental conflict model instead of a static "This file has merge conflicts": it shows a live count of remaining conflict blocks that updates as you hand-edit the markers (and disappears once the last one is gone, without waiting for a save), and offers "Accept <side>" links labelled with jj's own commit descriptions for each side, alongside the existing "Open Merge Tool" link. ([#112](https://github.com/kkkev/jj-idea/issues/112))
- Visible heads with no bookmark on them — work left behind after a forgotten `jj bookmark advance` — are now surfaced instead of silently sitting unnoticed: the bookmarks panel gets a collapsible "Unbookmarked heads" category (below the "@" row, above Local) listing each one as "\<closest bookmark\> +n \<change id\>", double-clickable to jump the log there, and the log table itself marks each such row with a small slashed-bookmark icon. ([#48](https://github.com/kkkev/jj-idea/issues/48), [#107](https://github.com/kkkev/jj-idea/issues/107))

### Changed
- The Working Copy tool window now has an icon representing a working copy rather than IntelliJ's default VCS icon ([#101](https://github.com/kkkev/jj-idea/issues/101))

### Fixed
- Restore, Show File History, Squash Selected Files, and Split no longer hide themselves for a deleted file selected alone in the Working Copy panel or a commit's changed-files list — a deleted file has no editor-visible content, and that was mistaken for "nothing selected". A selection mixing a deleted file with an edited one now restores both instead of silently dropping the deleted one. ([#122](https://github.com/kkkev/jj-idea/issues/122))
- The bookmark widget's/bookmarks panel's "\<name\> +N" indicator could show a wildly inflated (or capped, e.g. "+1000+") distance for a merge commit with one bookmarked parent and one unrelated, unbookmarked parent — it was counting the entire unrelated branch's history instead of the actual hops back to the bookmark.
- Scrolling the (early-access) paged log through a large repo's history got progressively slower the deeper you scrolled — each "Load more" page re-sorted and re-laid-out the entire log loaded so far, not just the new page, turning a scroll to depth N into roughly N² work. Loading a page now extends the previous merge/layout instead of recomputing it from scratch, so a long scroll costs roughly the same total work regardless of how many pages came before. ([#69](https://github.com/kkkev/jj-idea/issues/69))
- A divergent (conflicted) local bookmark couldn't be moved back onto the change it was already on — the natural way to resolve it — from either "Move Bookmark Here…" or "Move '\<bookmark\>' to Change…"; both dialogs assumed a bookmark has exactly one current target and excluded or misclassified it whenever a candidate matched. Every target is now offered, labelled "Resolve conflict", and selectable without ticking "Allow backward or sideways move". "Move '\<bookmark\>' to Change…" also now shows an undo balloon and supports Undo after a move, matching every other bookmark-move path. ([#121](https://github.com/kkkev/jj-idea/issues/121))
- A bookmark whose local ref was pending deletion (`jj bookmark delete`) but whose remote-tracking ref still existed could show a wildly wrong ahead count (e.g. "1000+ commits ahead") in the bookmarks panel — jj reports that count as the distance from the now-absent local ref to the repo root, not a real divergence. The log table already corrected this; the bookmarks panel, and every log-table refresh path past the very first load (paging, Refresh, search), now do too. ([#110](https://github.com/kkkev/jj-idea/issues/110))
- Toolbar Refresh now also re-reads bookmarks, tags, and working-copy state, not just log rows — previously an external change picked up only by the (unreliable on some network drives) op_heads file watch could leave the bookmark widget, bookmark chips, and reference filter stale even after clicking Refresh. ([#115](https://github.com/kkkev/jj-idea/issues/115))
- A conflicted file at a historical (non-working-copy) commit whose path contained a space was silently missing its conflict marker in that commit's changes list — the underlying `jj resolve --list` parser now reuses the same shape-aware parser as the working copy instead of a whitespace-splitting one, and populates the conflict registry for that revision too (laying groundwork for a future revision-aware conflicts view).
- Split's dialog now names the two resulting sides by what they *are* — "Existing commit"/"New commit" — consistently across the description labels, diff preview titles, summary line, and hunk picker, instead of a mix of DAG position ("Parent"/"Child"/"First"/"Second"/"Here") and a mismatched "(keeps change ID)" aside that only appeared on one of them; ticking a file now has the same meaning everywhere. The previous separate bold identity header and plain description sub-label above each description editor are combined into one label ("Description for existing commit (`<id>`)" / "Description for new commit (child of / sibling of / parent of `<id>`)"), dropping the redundant "(unchanged unless edited)" aside. A muted note under "Source" states the resulting shape, leading with the same "existing commit" naming, including that existing children become merges of both siblings in a parallel split, and word-wraps instead of forcing the dialog's left column wider. Every change id shown in these labels now uses the same bold-prefix/grey-remainder styling as the rest of the plugin. Toggling "Create parallel commits" also now relabels the description labels, note, and summary immediately, instead of only on the next file-tick. ([#101](https://github.com/kkkev/jj-idea/issues/101), [#114](https://github.com/kkkev/jj-idea/issues/114))
...
- Bookmark rows in the bookmarks panel now support the log's own change actions — New Change, New Change..., Edit Change, Rebase, and Duplicate — via right-click, including multi-select (Ctrl+Click several bookmark rows and "New Change From These" creates a merge with all of them as parents). Tag rows in the same panel gained the same actions. ([#48](https://github.com/kkkev/jj-idea/issues/48), [#108](https://github.com/kkkev/jj-idea/issues/108))
- Every bookmark row action (Delete/Forget/Rename/Advance/Push to all remotes/Track/Untrack, plus Filter Log to Bookmark and Navigate Log to Bookmark) is now registered and keymap-assignable in Settings → Keymap, shows a shortcut hint in the bookmarks panel's own right-click menu once bound, and double-clicking a bookmark row navigates to its change the same way double-clicking a log row shows its diff. ([#48](https://github.com/kkkev/jj-idea/issues/48), [#109](https://github.com/kkkev/jj-idea/issues/109))
- A local bookmark ahead of or behind its tracked remote now shows the same `↑n`/`↓m` divergence indicator on its own leaf in the bookmarks panel that a remote bookmark already showed — previously it always read 0/0 regardless of the actual state. With more than one tracked remote, the indicator shows the largest divergence rather than summing across remotes; per-remote detail is a follow-up. ([#48](https://github.com/kkkev/jj-idea/issues/48), [#110](https://github.com/kkkev/jj-idea/issues/110))
- "Open Jujutsu Log" is now reachable from VCS → Jujutsu and from the VCS Operations popup, instead of only via Find Action or a manually-bound shortcut; invoking it while the log tab is already open but the tool window is hidden now brings it to the front instead of appearing to do nothing. ([#118](https://github.com/kkkev/jj-idea/issues/118))

### Fixed
- Splitting a change with zero whole files ticked but one or more files partially picked via "Pick Hunks…" was rejected with "Move at least one file to the child" — hunks-only splits (nothing fully ticked) now work as expected. ([#117](https://github.com/kkkev/jj-idea/issues/117))
- The bookmarks panel always showed remote bookmarks with the "tracked" icon and never showed ahead/behind divergence, regardless of the actual state — an untracked remote bookmark's context menu also always offered "Untrack" instead of "Track". ([#48](https://github.com/kkkev/jj-idea/issues/48), [#110](https://github.com/kkkev/jj-idea/issues/110))
- A conflicted or divergent local bookmark (e.g. moved to different targets by two concurrent operations) vanished from the bookmarks panel entirely, even though the log table still showed it with its conflict marker — the same defect affected conflicted tags. ([#110](https://github.com/kkkev/jj-idea/issues/110))
- A stale jj workspace (`jj workspace update-stale`) no longer breaks the Working Copy tool window, or shows a wrong/missing message instead of the right one — this now covers essentially every command and dialog, not just the Working Copy panel: Advance/Rename/Create/Move Bookmark, Set Tag, Push, Show History, and the Split/Squash/Rebase/New Change dialogs' commit pickers. All now offer a one-click "Update Stale Workspace" action that repairs it and automatically finishes whatever you were doing. (Annotate still reports the error as before — a fix attempt there didn't pan out.)
- Renaming or creating a bookmark could show a flatly wrong "There is already a bookmark with that name" error for any failure at all (e.g. an invalid target revision) — it now only shows that message when jj actually reports a name conflict.
- The Move Bookmark dialog could warn that a move was "backward or sideways" even when it was genuinely forward, if the classification query failed for any reason (including a stale workspace) — it now surfaces the failure instead of silently guessing backward.
- A large changes tree with a Jujutsu repository root visible could freeze the UI for tens of seconds — the repo-root icon lookup was doing a blocking VCS-root resolution from inside the tree's paint path on every repaint.
- A commit whose parent has been filtered out of view now shows a wiggly line down from its circle instead of no connector at all — which looked identical to a true repository root.
- A commit whose parent falls beyond the configured log limit now shows a faded straight line down from its circle instead of no connector at all — which looked identical to a true repository root. A commit whose parent has been filtered out of view instead shows a wiggly line, since that's genuine elision (like jj's own `~`) rather than history that simply hasn't loaded yet — the two previously looked the same. A merge with one loaded parent and one unresolved parent now shows its stub too, in its own lane alongside the real connector.
...
- Every mutating command (New Change, Edit, Abandon, Rebase, Squash, Split, Duplicate Onto, Restore, Undo, resolving a merge conflict, switching working copies) could briefly freeze the UI on a large repository, tripping IntelliJ's "slow operation may freeze UI" assertion — the post-command VFS refresh's recursive directory walk ran synchronously on the EDT despite requesting an async refresh; it now runs on a background thread. ([#113](https://github.com/kkkev/jj-idea/issues/113))
- On IntelliJ IDEA 2025.1, running any jj command could crash with "Access is allowed from write thread only" — the pre-command document-save check queried whether writing was allowed before acquiring the write-intent lock that check itself required on that platform version.
- Resolving a rebase conflict could show your own change as "Theirs" and the commit you rebased onto as "Yours" — which side jj renders as full content vs. a diff varies per conflict, not per operation, so a fixed guess was right about half the time. The merge tool now reads jj's own per-side label from the conflict markers and names the panes after the actual commits (falling back to "Side #1"/"Side #2" when markers carry no commit info), and orients "Yours" to your own change; merge and squash conflicts are unaffected. ([#112](https://github.com/kkkev/jj-idea/issues/112))
- Double-clicking a conflicted file's diff for any commit other than the working copy showed "Cannot show diff: Could not extract conflict data" (or, worse, silently showed the working copy's unrelated conflict) — the diff always read the file from disk regardless of which commit's diff was opened. It now reads that commit's own conflicted content, so browsing the log to a conflicted commit shows its own three-way conflict, whether it's an ancestor of the working copy, a descendant that inherited the conflict, or an unrelated commit. ([#119](https://github.com/kkkev/jj-idea/issues/119))
- The log graph got measurably slower to repaint and to hover the further down a long log you scrolled — drawing a row's connector lines rescanned every row above it, and hovering a long edge rebuilt its whole lane index on every mouse move and scroll tick instead of reusing the cache already built for it. Both now do constant work per row/event regardless of scroll position or log size.
...
- The Diff Base setting (Settings → Version Control → Jujutsu, and the new quick action above) gained a fourth option, "Previous commit (grandparent)" — the commit before your last one, or trunk if you're right above it. ([#43](https://github.com/kkkev/jj-idea/issues/43))
...
### Fixed
- The Describe dialog's prompt now shows the short change id (matching the id shown everywhere else in the UI) instead of the full 64-character id. ([#76](https://github.com/kkkev/jj-idea/issues/76))
...
- The bookmark toolbar widget and both status bar widgets no longer show up in projects that aren't Jujutsu repositories — they used to appear (as an empty, non-functional widget) in any project as soon as the plugin was installed. They now also appear immediately after running Jujutsu → Init, without needing to restart the IDE. ([#100](https://github.com/kkkev/jj-idea/issues/100))
- Fetching from a remote could log a "Write-unsafe context!" IDE error and, more importantly, silently skip saving your edited files before the jj command ran, so it could operate on stale on-disk content — a regression from an earlier fix for a shutdown hang. Saving is now legal again in all cases, and a modal dialog still can't block indefinitely while it waits. ([#106](https://github.com/kkkev/jj-idea/issues/106))
- "Move Bookmark Here..." and "Move Bookmark to Change..." no longer open with OK greyed out when a valid destination is already selected — the OK button now reflects the pre-selected row immediately, instead of only updating after you change the selection or tick "Allow backwards or sideways move". ([#105](https://github.com/kkkev/jj-idea/issues/105))
- Settings → Version Control → Jujutsu now shows the identity actually in effect for each repository — including one set by a jj config scope (e.g. `--when.repositories`) — and no longer prompts to configure an identity a scope already supplies. The global "User Identity" fields now read and write the same (user-level) scope, instead of showing a merged value they couldn't actually update. ([#89](https://github.com/kkkev/jj-idea/issues/89))
- The log's right-click menu, the bookmark widget dropdown, and the bookmarks panel's `@` node now say "Advance Bookmark to Working Copy" instead of "Advance Bookmark Here" — in a per-row menu, "Here" read as "move the bookmark to this row," but the action always advances the nearest bookmark to your current working copy, never to the clicked row. ([#104](https://github.com/kkkev/jj-idea/issues/104))
...
- A one-time, dismissible notification now tells you when upgrading your `jj` executable would unlock additional plugin features (starting with Advance Bookmark, which needs jj 0.39+), with a link straight to the settings page to update the executable path.
- Abandoning a change now shows an "Undo" link right in the confirmation notification. If you dismiss it, the VCS menu keeps a persistent "Undo Abandon" entry so you can still undo it later — it only ever undoes the specific abandon you just did, never anything a background process or another window did in the meantime.
...
- The "Squash Into..." dialogs now show a read-only diff preview for the selected file, matching Split's preview: a "Before" pane and a "Destination" pane, updating as you click files or tick/untick them.
- The "Squash Into..." dialogs gained a "Pick Hunks…" button, letting you squash only part of a file's change into the destination while the rest stays in the source — the same three-pane arrow picker Split already uses. Only available when squashing a single source; the file shows half-checked afterward, and unchecking/rechecking discards the partial pick.
- "New Change..." in the log's right-click menu now opens a dialog offering a description, a placement choice (a plain child — jj's default — or inserting the new change before/after the selected commit(s), rebasing the affected descendants onto it), and whether the working copy follows, with a live preview of the resulting graph before you commit to it. ([#83](https://github.com/kkkev/jj-idea/issues/83))
...
- The commit details panel no longer shows a stale diff after selecting a different commit that touches the same file(s) as the previously selected one — the tab title updated but the diff content kept showing the old selection's changes.
