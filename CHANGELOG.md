...
- Issue-tracker references (e.g. `JIRA-123`) *inside* a bookmark or tag name (e.g. a bookmark named `jira-123-fix-thing`) are now their own clickable link within the chip, separate from the chip's own right-click menu — link-colored, hand cursor and underline only on that exact reference, opening the same as elsewhere on left-click. ([#15](https://github.com/kkkev/jj-idea/issues/15))
## [0.8.5] - 2026-08-02
