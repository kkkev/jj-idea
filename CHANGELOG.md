...
* The Working Copy tool window's changes tree now supports IntelliJ's built-in Reformat Code (<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>L</kbd>) and Optimize Imports (<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>O</kbd>) shortcuts on the selected file(s), matching the built-in Git/Commit changes view.
