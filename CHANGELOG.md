...
- The log and file-history toolbars' eye-icon menu (now called "View Options") has a new "Commit Tooltips" tick-box to turn off the row hover tooltip, for anyone who finds it more distracting than useful now that the details pane covers the same info on selection — the column-visibility and details-position toggles that used to live in two separate submenus now live alongside it in one flat menu. ([#51](https://github.com/kkkev/jj-idea/issues/51), [#75](https://github.com/kkkev/jj-idea/issues/75))
- Three new settings under Settings → Version Control → Jujutsu:
  - A checkbox to turn off the alternating row colors in the log and in the Duplicate/Squash/Rebase picker tables. ([#79](https://github.com/kkkev/jj-idea/issues/79))
