...
class JujutsuLogProvider : VcsLogProvider {
    ...
    override fun readFirstBlock(
        ...
        val limit = requirements.commitCount.takeIf { it > 0 }
        val result = root.jujutsuRepository.logService.getLog(limit = limit)
    ...
    override fun readAllHashes(root: VirtualFile, consumer: Consumer<in TimedVcsCommit>): VcsLogProvider.LogData {
        ...
        val result = root.jujutsuRepository.logService.getCommitGraph(limit = 1000)
    ...
    override fun readFullDetails(root: VirtualFile, hashes: List<String>, consumer: Consumer<in VcsFullCommitDetails>) {
        ...
        hashes.forEach { hexHash ->
            try {
                ...
                val result = logService.getLog(Expression(changeId.full))
    ...
    override fun getCommitsMatchingFilter(
        ...
        val limit = if (maxCount > 0) maxCount else 1000
        val result = root.jujutsuRepository.logService.getCommitGraph(limit = limit)
    ...
    override fun <T : Any?> getPropertyValue(property: VcsLogProperty<T?>?): T? = when (property) {
