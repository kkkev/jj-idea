...
object DescriptionRenderingStyle {
     ...
     */
    fun getTextAttributes(description: Description, isWorkingCopy: Boolean): SimpleTextAttributes {
        return when {
            description.empty && isWorkingCopy -> SimpleTextAttributes(
    ...
    fun getTextAttributes(entry: LogEntry): SimpleTextAttributes {
        return getTextAttributes(entry.description, entry.isWorkingCopy)
    }

    /**
     * Determine the font style for Graphics2D rendering.
     */
    fun getFontStyle(description: Description, isWorkingCopy: Boolean): Int {
        return when {
     ...
     */
    fun getFontStyle(entry: LogEntry): Int {
        return getFontStyle(entry.description, entry.isWorkingCopy)
    }

    /**
     * Determine the text color for Graphics2D rendering.
     */
    fun getTextColor(description: Description, isSelected: Boolean, selectionForeground: Color, defaultForeground: Color): Color {
