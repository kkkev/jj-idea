...
class SquashIntoDialog(
    ...
    // Candidate picker — search + log-style table (jj-idea-tq4b). For predefined-candidates mode
    // (parent mode), autoLoad is off: the fixed candidate list is pushed in directly via
    // setupPredefinedCandidates() instead of loading/searching the whole repo.
    private val picker: CommitPickerPanel = CommitPickerPanel(
        project = project,
        repo = repo,
        searchPlaceholder = JujutsuBundle.message(
            if (pickingSources) "dialog.squash.into.source.search" else "dialog.squash.into.destination.search"
        ),
        multiSelect = pickingSources,
        autoLoad = !hasPredefinedCandidates,
        onInitialLoad = { if (pickingSources) picker.selectFirstMatching { it.isWorkingCopy } }
    )
    internal val pickerTable get() = picker.table
    ...
    override fun createCenterPanel(): JComponent {
        ...
        if (hasPredefinedCandidates) {
            // No search bar in this mode (autoLoad = false), so the picker's height is just the
            // fixed candidate list's table height.
            val fixedHeight = mode.candidates!!.size * pickerTable.rowHeight + JBUI.scale(4)
            picker.preferredSize = Dimension(0, fixedHeight)
            picker.maximumSize = Dimension(Int.MAX_VALUE, fixedHeight)
        }
        ...
        picker.reload { entry ->
            when (mode) {
                is SquashMode.PickDestination -> {
                    val excluded =
                        RebaseSimulator.excludedDestinationIds(picker.entries, sourceIds, RebaseSourceMode.REVISION)
                    entry.id !in excluded && !entry.immutable
                }

                is SquashMode.PickSources -> entry.id != mode.destination.id && !entry.immutable
            }
        }
    }
    ...
    private fun selectedDestinationId(): ChangeId? = picker.selectedEntries().firstOrNull()?.id

    private fun selectedDestinationEntry(): LogEntry? = picker.selectedEntries().firstOrNull()

    private fun selectedSourceEntries(): List<LogEntry> = picker.selectedEntries()
