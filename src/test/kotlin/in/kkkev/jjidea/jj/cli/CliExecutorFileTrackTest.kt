...
class CliExecutorFileTrackTest {
    ...
    fun `fileTrackArgs - single path`() {
        fileTrackArgs(listOf("foo.txt")) shouldBe listOf("file", "track", "--include-ignored", "cwd:\"foo.txt\"")
    }
    ...
    fun `fileUntrackArgs - single path`() {
        fileUntrackArgs(listOf("foo.txt")) shouldBe listOf("file", "untrack", "cwd:\"foo.txt\"")
    }
