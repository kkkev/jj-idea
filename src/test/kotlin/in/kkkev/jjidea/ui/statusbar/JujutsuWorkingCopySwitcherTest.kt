...
class JujutsuWorkingCopySwitcherTest {
    ...
    fun `bookmarks appear before log entries`() {
        ...
        items[0].shouldBeInstanceOf<JujutsuWorkingCopySwitcher.BookmarkEntry>()
        (items[0] as JujutsuWorkingCopySwitcher.BookmarkEntry).displayName shouldBe "my-feature"
        items[1].shouldBeInstanceOf<JujutsuWorkingCopySwitcher.ChangeEntry>()
    }
    ...
    fun `working copy entry is excluded from changes section`() {
        ...
        val changeEntries = items.filterIsInstance<JujutsuWorkingCopySwitcher.ChangeEntry>()
    ...
    fun `remote bookmarks are excluded`() {
        ...
        val bookmarkEntries = items.filterIsInstance<JujutsuWorkingCopySwitcher.BookmarkEntry>()
    ...
    fun `bookmark immutability is cross-referenced from log entries`() {
        ...
        val bookmark = items.filterIsInstance<JujutsuWorkingCopySwitcher.BookmarkEntry>().first()
    ...
    fun `change entry display name uses short id and description`() {
        ...
        val change = items.filterIsInstance<JujutsuWorkingCopySwitcher.ChangeEntry>().first()
    ...
    fun `recent changes are limited to 10`() {
        ...
        items.filterIsInstance<JujutsuWorkingCopySwitcher.ChangeEntry>() shouldHaveSize 10
    }
